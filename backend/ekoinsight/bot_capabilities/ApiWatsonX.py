...
class ApiWatsonX(ApiIbm):
    ...
    def fetch_english_prompt(self,item="metal can",prompt_template='pollution_prompt_template',max_tokens=300,stop_sequences=['END']):
        #basically no mention of language
        if not self.dry_run:
    ...
    def fetch_prompt(self,item="metal can",prompt_template='pollution_prompt_template',max_tokens=300,stop_sequences=['END']):
        if not self.dry_run:
            if not prompt_template.startswith=="ibm_":
                prompt_template="ibm_"+prompt_template
